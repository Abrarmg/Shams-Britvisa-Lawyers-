      ...
      <main className="pt-32 pb-24">
        ...
        <section className="container mx-auto px-6 mb-24">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <Link 
                to="/booking"
                className="inline-flex items-center gap-2 bg-brand-red text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-red-700 transition-all shadow-lg hover:shadow-red-500/20"
              >
        ...
        <section className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-12">
              ...
              >
                <h2 className="text-3xl font-black text-brand-black mb-6 flex items-center gap-3">
                ...
                <ul className="space-y-4">
                  {whoIsItFor.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-4">
                      <div className="w-6 h-6 bg-brand-red/10 text-brand-red rounded-full flex items-center justify-center shrink-0 mt-1">
                        <Check size={14} />
                      </div>
            ...
            <div className="space-y-12">
              <motion.div
                ...
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                className="bg-brand-black text-white p-10 rounded-[40px] shadow-2xl"
              >
                ...
                <ul className="space-y-6">
                  {keyRequirements.map((req, idx) => (
                    <li key={idx} className="flex items-start gap-4">
                      <div className="w-2 h-2 bg-brand-red rounded-full mt-2.5 shrink-0" />
                      <span className="text-white/80 font-medium text-lg leading-relaxed">{req}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
        ...
        <section className="container mx-auto px-6 mt-24">
          ...
          >
            ...
            <div className="overflow-hidden rounded-3xl border border-zinc-200 shadow-xl bg-white">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-brand-red text-white">
                    <th className="py-6 px-8 font-black uppercase tracking-wider text-sm">Fee Type</th>
                    <th className="py-6 px-8 font-black uppercase tracking-wider text-sm">Amount</th>
                    <th className="py-6 px-8 font-black uppercase tracking-wider text-sm">Notes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100">
                  {fees.map((fee, idx) => (
                    <tr key={idx} className="hover:bg-zinc-50 transition-colors">
                      <td className="py-6 px-8 font-bold text-brand-black">{fee.type}</td>
                      <td className="py-6 px-8 font-black text-brand-red">{fee.amount}</td>
                      <td className="py-6 px-8 text-zinc-500 font-medium">{fee.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="container mx-auto px-6 mt-24">
          <div className="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto">
            <motion.div
              ...
              className="bg-white p-10 rounded-[40px] shadow-xl border border-zinc-100"
            >
              <h2 className="text-2xl font-black text-brand-black mb-6 flex items-center gap-3">
            ...
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="bg-brand-red/5 p-10 rounded-[40px] border border-brand-red/10"
            >
              <h2 className="text-2xl font-black text-brand-red mb-6">How We Can Help</h2>
